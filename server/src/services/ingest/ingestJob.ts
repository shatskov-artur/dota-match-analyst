...
import {
  ...
  deriveGameState,
  type ValveGames,
  type EnrichedLiveGame,
} from '../liveAggregator.js'
...
export function isIngestDisabled(): boolean {
  ...
  // Both now come through the zod schema — read straight off process.env, a misspelled
  // kill switch failed silently, which is the one failure a kill switch must not have.
  return env.INGEST_DISABLED === '1' || env.HISTORY_SAMPLER_DISABLED === '1'
}
...
/**
 * Stage 1 — the live feed.
 *
 * Snapshots every archivable match and, via enrichLiveGames, keeps the Redis gold/XP
 * timeseries fed for the whole live list. Returns the enriched games so the sync stage can
 * contribute live league ids; when Valve is unreachable this throws and the caller carries
 * on with an empty list, which is the honest degraded answer rather than a special case.
 */
async function runLiveSnapshotStage(result: TickResult): Promise<EnrichedLiveGame[]> {
...
/**
 * Stage 2 — bracket, schedule, standings and match discovery.
 *
 * Reads dota2.com and OpenDota, never Valve's live feed, so it is independent of stage 1
 * and runs on whatever stage 1 managed to produce — including nothing.
 */
async function runTournamentSyncStage(tick: number, enriched: EnrichedLiveGame[]): Promise<void> {
  /*
  ...
  const synced = await syncLeagues([...trackedLeagueIds, ...ofInterest, ...liveLeagues], ofInterestSet)
  if (synced.length > 0) {
    logger.info(
...
/** Stage 3 — the stale-live sweep and OpenDota backfill. Reads OpenDota only. */
async function runBackfillStage(tick: number): Promise<void> {
...
/*
 * THREE independent stages, each with its own catch, and that separation fixes a real
 * outage shape rather than being tidiness.
 *
 * All of this used to sit in one try block whose first statement was the Valve fetch. So
 * when Valve answered 5xx and the 24h stale copy had expired, the throw skipped the whole
 * tick — including the bracket sync, which reads dota2.com, and the backfill, which reads
 * OpenDota. Two upstreams that were perfectly healthy went quiet because a third was down,
 * which is exactly the silence leaguesOfInterest.ts was written to prevent.
 */
export async function runOnce(): Promise<TickResult> {
  ...
  // The outer try/finally exists only to clear isRunning. It is the overlap guard, and a
  // throw that left it set would disable the job for the rest of the process's life —
  // every later tick would log "overlap, skipping" and do nothing at all.
  try {
    let enriched: EnrichedLiveGame[] = []
    try {
      enriched = await runLiveSnapshotStage(result)
    } catch (err) {
      logger.error({ err: briefError(err) }, 'ingest: live snapshot stage failed')
    }

    if (tick % TOURNAMENT_SYNC_EVERY === 0) {
      try {
        await runTournamentSyncStage(tick, enriched)
      } catch (err) {
        logger.error({ err: briefError(err) }, 'ingest: tournament sync stage failed')
      }
    }

    try {
      await runBackfillStage(tick)
    } catch (err) {
      logger.error({ err: briefError(err) }, 'ingest: backfill stage failed')
    }
