import MatchCard from './MatchCard'
import type { EnrichedGame } from '../hooks/useLiveGames'
import { getStatusLabel } from '../utils/gameState'

const STATUS_ORDER: Record<string, number> = {
  'Live': 0, 'Starting': 1, 'Strategy': 2, 'Draft': 3,
  'Waiting': 4, 'Break': 5, 'Post-game': 6, 'Unknown': 7,
}

interface GroupedLeague {
  leagueId: number
  leagueName: string
  matches: EnrichedGame[]
}

interface MatchBentoGridProps {
  grouped: GroupedLeague[]
}

/**
 * Neon Bento home grid (replaces LeagueAccordion). Flattens all leagues into a
 * single bento grid sorted by live-ness; the top match becomes the featured 2x2
 * tile. League identity is carried on each card's label, so grouping is preserved
 * visually without a collapsing accordion.
 *
 * Grid: 1 col (phone) -> 2 col (tablet) -> 3 col (laptop+), with grid-auto-rows so
 * the featured 2x2 tile lines up cleanly (ui-ux-pro-max Bento Box Grid pattern).
 */
export default function MatchBentoGrid({ grouped }: MatchBentoGridProps) {
  const all = grouped.flatMap(g =>
    g.matches.map(m => ({ ...m, league_name: m.league_name || g.leagueName })),
  )

  const sorted = [...all].sort((a, b) => {
    const ao = STATUS_ORDER[getStatusLabel(a.game_state, a.scoreboard)] ?? 3
    const bo = STATUS_ORDER[getStatusLabel(b.game_state, b.scoreboard)] ?? 3
    return ao - bo
  })

  if (sorted.length === 0) return null

  const [featured, ...rest] = sorted

  return (
    <div
      className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 py-6
                 sm:[grid-auto-rows:minmax(150px,auto)] lg:[grid-auto-rows:minmax(160px,auto)]"
    >
      {/* Featured tile: full-width on phone, 2 cols on tablet, 2x2 on laptop+ */}
      <div className="flex sm:col-span-2 lg:col-span-2 lg:row-span-2">
        <MatchCard game={featured} featured />
      </div>
      {rest.map(game => (
        <MatchCard key={game.match_id} game={game} />
      ))}
    </div>
  )
}
